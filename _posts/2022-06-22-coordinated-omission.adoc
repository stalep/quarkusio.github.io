...
++++
include::../assets/images/posts/coordinated-omission/insights_video.html[]
++++
...
Here's the thing, it is very difficult! The maximum value will be the same
...
Coordinated Omission is the unintended back pressure a system under test can apply to a load generation tool, that prevents that tool for accurately recording user experience.  A load generation tool that suffers from coordintated omission 
